#include "doubly_linear_linked_list.h"
...
{
    node_t* p_run = get_node(p_list, search_data);
    if(p_run == NULL)
        return FALSE;
    else
        return TRUE;
...
{
    node_t* p_remove_node = get_node(p_list, remove_data);
